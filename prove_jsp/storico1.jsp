	...
	String query = "SELECT * FROM silos ORDER BY data DESC LIMIT 0,100";
    ...
    for (int i=(energies.size()-1); i>=0; --i){
		...
		dataset.addValue(loads.get(i), "Carico [kG]", dates.get(i).substring(14));
		dataset.addValue(energies.get(i), "Potenza [kW]", dates.get(i).substring(14));
	}

	// Crea il grafico
	JFreeChart chart = ChartFactory.createLineChart(
		"Storico sensori Silos (ultimi 100 rilevamenti)", // chart title
		"", // domain axis label
		"", // range axis label
		dataset, // data
	...
	int max_range = Collections.max(loads) + 50;
